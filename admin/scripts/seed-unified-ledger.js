...
async function main() {
  console.log('🔄 Bắt đầu xây dựng Sổ Kế Toán Ví Thống Nhất 100%: Link Task + Tính Biến Động Số Dư...');
  ...
  let orderCodeCounter = BigInt(2026084001);
  ...
  for (let i = 0; i < TOTAL_TASKS; i++) {
    ...
    // Step 1: Poster Deposit via PayOS
    if (posterWallet) {
    ...
    // Step 4: Record ESCROW_HOLD for Poster linked to Task Title
    if (posterWallet) {
      await prisma.walletTransaction.create({
        data: {
          walletId: posterWallet.id,
      ...
      if (escrowStatus === 'HOLDING') {
        ...
        // RELEASED: Poster's balance and available balance are permanently deducted
        await prisma.wallet.update({
          where: { id: posterWallet.id },
          data: {
        ...
        // Tasker Receives 92% Net (ESCROW_RELEASE)
        if (taskerWallet) {
          await prisma.walletTransaction.create({
            data: {
        ...
        // REFUND: Refund transaction logged for Poster
        await prisma.walletTransaction.create({
          data: {
            walletId: posterWallet.id,
            type: 'REFUND',
  ...
  console.log('\n==================================================');
  console.log('✅ HOÀN TẤT ĐỒNG BỘ NGUYÊN BẢN SỔ KẾ TOÁN VÍ & KÝ QUY TRỪ SỐ DƯ!');
