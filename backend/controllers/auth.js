    ...
    if (admin.apps.length > 0 && updatedUser.firebaseUid) {
      ...
      if (!hasServiceAccount) {
        ...
        return res.status(500).json({ 
          message: 'Không thể reset mật khẩu: Backend của bạn thiếu cấu hình biến môi trường FIREBASE_CLIENT_EMAIL hoặc FIREBASE_PRIVATE_KEY (Service Account) để đồng bộ mật khẩu lên Firebase.' 
        });
      }
