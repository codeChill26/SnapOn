...
async function sendResetPasswordEmail(to, otp) {
  try {
          ...
          <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #FFFFFF; border-radius: 16px; overflow: hidden; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.05); border: 1px solid #EAEAEA;">
            ...
            <tr>
              <td style="padding: 20px 40px 40px 40px;">
                ...
                <div style="text-align: center; margin: 32px 0;">
                  <div style="display: inline-block; padding: 20px 40px; background-color: #FFF5F0; border: 1.5px dashed #FF6600; border-radius: 12px; text-align: center; box-shadow: inset 0 2px 4px rgba(255, 102, 0, 0.02);">
                    <span style="display: block; font-size: 36px; font-weight: 800; letter-spacing: 8px; font-family: 'Courier New', Courier, monospace; color: #FF6600;">${otp}</span>
                  </div>
                </div>
                ...
                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                  <tr>
                    <td style="text-align: center; color: #9CA3AF; font-size: 12px; line-height: 20px;">
                      This is an automated email from the system. If you did not request this code, please ignore this email.<br />
                      &copy; 2026 SnapOn App. All rights reserved.
                    </td>
                  </tr>
                </table>
              </td>
    ...
    const data = await response.json();
    console.log(`[EMAIL SERVICE] Reset email sent successfully: ${data.messageId}`);
    return { success: true, messageId: data.messageId };
...
async function sendDeletionOtpEmail(to, name, otp) {
  try {
    if (!process.env.BREVO_API_KEY) {
      throw new Error('BREVO_API_KEY is not configured');
    }
    ...
    const payload = {
      ...
      subject: `[SnapOn] Mã xác thực yêu cầu xóa tài khoản: ${otp}`,
      htmlContent: `
        <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #F8F9FA; padding: 50px 0; margin: 0; width: 100%;">
          <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #FFFFFF; border-radius: 16px; overflow: hidden; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.05); border: 1px solid #EAEAEA;">
            <tr>
            ...
            <tr>
              <td style="padding: 20px 40px 40px 40px;">
                ...
                <p style="color: #4B5563; font-size: 15px; line-height: 24px; margin: 0 0 32px 0;">
                  Bạn nhận được email này vì bạn hoặc ai đó đã gửi yêu cầu xóa tài khoản trên hệ thống SnapOn. Đây là mã OTP để xác nhận hành động này. Nếu bạn không gửi yêu cầu này, vui lòng bỏ qua email.
                </p>
                <div style="text-align: center; margin: 32px 0;">
                  <div style="display: inline-block; padding: 20px 40px; background-color: #FFF5F0; border: 1.5px dashed #FF6600; border-radius: 12px; text-align: center;">
                    <span style="display: block; font-size: 12px; font-weight: 700; text-transform: uppercase; color: #C2410C; margin-bottom: 8px;">Mã xác thực OTP xóa tài khoản</span>
                    <span style="display: block; font-size: 36px; font-weight: 800; letter-spacing: 8px; font-family: 'Courier New', Courier, monospace; color: #FF6600;">${otp}</span>
                  </div>
                </div>
                <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #FFF5F5; border-radius: 8px; margin: 32px 0 0 0;">
                  <tr>
                    <td style="padding: 12px 16px; text-align: center; color: #DC2626; font-size: 13px; font-weight: 500; line-height: 18px;">
                      ⚠️ Mã xác thực này có hiệu lực trong vòng <strong>5 phút</strong>. Hành động xóa tài khoản không thể hoàn tác, vui lòng cẩn trọng.
                    </td>
                  </tr>
                </table>
                <hr style="border: 0; border-top: 1px solid #E5E7EB; margin: 40px 0 24px 0;" />
    ...
    const data = await response.json();
    console.log(`[EMAIL SERVICE] Deletion OTP email sent successfully: ${data.messageId}`);
    return { success: true, messageId: data.messageId };

  } catch (error) {
    console.error('[EMAIL SERVICE] Error sending deletion OTP email:', error);
    return { success: false, message: error.message };
  }
}
